...
class MojoUtils
{
    ...
    static ProxyConfig getProxyConfig(MavenSession mavenSession, SettingsDecrypter decrypter)
    {
        if ((mavenSession == null) || (mavenSession.getSettings() == null) ||
                (mavenSession.getSettings().getProxies() == null) || mavenSession.getSettings().getProxies().isEmpty())
        {
        ...
        else
        {
            ...
            for (Proxy mavenProxy : mavenProxies)
            {
                if (mavenProxy.isActive())
                {
                    ...
                    proxies.add(new ProxyConfig.Proxy(mavenProxy.getId(), mavenProxy.getProtocol(),
                            mavenProxy.getHost(), mavenProxy.getPort(), mavenProxy.getUsername(),
                            mavenProxy.getPassword(), mavenProxy.getNonProxyHosts()));
                }
            }
